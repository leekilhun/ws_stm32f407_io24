...
void cmdInit(cmd_t *p_cmd)
{
  ...
  memset(&p_cmd->rx_packet.buffer[0], 0, CMD_MAX_DATA_LENGTH + 8);
  memset(&p_cmd->tx_packet.buffer[0], 0, CMD_MAX_DATA_LENGTH + 8);
...
bool cmdReceivePacket(cmd_t *p_cmd)
{
  ...
  case CMD_STATE_WAIT_LENGTH_H:
    ...
    else if (p_cmd->rx_packet.length < CMD_MAX_DATA_LENGTH)
    {
      p_cmd->index = 0;
  ...
  case CMD_STATE_WAIT_DATA:
    ...
    else if (p_cmd->index >= CMD_MAX_DATA_LENGTH)
    {
      p_cmd->state = CMD_STATE_WAIT_STX;
    }
    break;
