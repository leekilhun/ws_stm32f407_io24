  ...
  switch(ch)
  {
...
#else
        if (HAL_UART_Receive_IT(&huart1, (uint8_t *)&uart_tbl[ch].data, 1) != HAL_OK)
        {
          ret = false;
        }
#endif
    ...
    case _DEF_UART3:
    {
      ...
      else
      {
        ...
        if (HAL_UART_Receive_IT(&huart2, (uint8_t *)&uart_tbl[ch].data, 1) != HAL_OK)
        {
          ret = false;
        }
      }
...
#ifdef _USE_HW_UART_DMA_1
#else
      qbufferWrite(&qbuffer[_DEF_UART2], &uart_tbl[_DEF_UART2].data, 1);
      HAL_UART_Receive_IT(&huart1, (uint8_t *)&uart_tbl[_DEF_UART2].data, 1);
#endif
    }
