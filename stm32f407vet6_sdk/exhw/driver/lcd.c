...
void lcdPrintf(int x, int y, uint16_t color,  const char *fmt, ...)
{
	...
	for( i=0; i<len-1; i++ )
	{
		str[i] = print_buffer[i];
	}

	lcd.setString(&str[0],len-1);
//	if (lcd.sendBuffer(&str[0],  (uint32_t)(len-1), 10) == false)
//	{
//		logPrintf("[err]sendBuffer");
//	}
...
void cliLcd(cli_args_t *args)
{
	...
	/* print cli*/
	if (args->argc == 3 && args->isStr(0, "print", false) == true)
	{
		uint8_t line = (uint8_t)args->getData(1);
		char *strMsg = args->getStr(2);

		if (lcdDrawAvailable() == true)
		{
			if(line==1) lcd.setCommand(0x80);
			else 				lcd.setCommand(0xC0);

			lcdPrintf(0,0,lightblue,"%s\n",strMsg);
		}
	...
	/* cmd cli*/
	if (args->argc == 2 && args->isStr(0, "cmd", false) == true)
	{

		//uint8_t cmdno = (uint8_t)args->getData(1);
	...
	if (ret != true)
	{
		cliPrintf("lcd print line[0:1] text[..max_char 16]\n");
		cliPrintf("lcd cmd [lcd1602] \n");
