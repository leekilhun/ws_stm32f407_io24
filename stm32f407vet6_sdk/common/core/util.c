...
/*
 * Transmit datatype BCD-> Decimal
 * */
uint8_t utilB2D(uint8_t bcdByte)
{
  uint8_t low, high;
  low = bcdByte & 0x0F;
  high = ((bcdByte >> 4) & 0x0F) * 10;
  return high + low;
}

/*
 * Transmit datatype Decimal -> BCD
 * */
...
uint8_t utilParseArgs(char *argStr, char **argv)
{
	...
	for (tok = strtok_r(argStr, DELIM_CHARS, &next_ptr); tok; tok = strtok_r(NULL, DELIM_CHARS, &next_ptr))
	{
		argv[argc++] = tok;
	}
